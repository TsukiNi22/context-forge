...
/* the instance should always be destroyed before the shared object (code of the destructor inside the .so) */
static void checkRule(utils::encapsulation::SharedObject& plugin, const std::string& name, const std::string& input)
{
    ...
    // without load a rule should never touch the content
...
TEST_P(PluginsTest, FactoryDistinctInstances) {
    ...
    forge::rules::IInstruction* (*factory)() = plugin.loadFunction<forge::rules::IInstruction* (*)()>("factory");
    std::unique_ptr<forge::rules::IInstruction> first(factory());
    std::unique_ptr<forge::rules::IInstruction> second(factory());
    ASSERT_NE(first.get(), nullptr);
    ASSERT_NE(second.get(), nullptr);
    ASSERT_NE(first.get(), second.get());
}

TEST_P(PluginsTest, InstanceNameMatchPluginName) {
    const PluginCase& testCase = GetParam();
    utils::encapsulation::SharedObject plugin(PluginsTest::path(testCase.path));

    // the debug name of the instance should be the one used by the rules (.cfg)
    forge::rules::IInstruction* (*factory)() = plugin.loadFunction<forge::rules::IInstruction* (*)()>("factory");
    std::unique_ptr<forge::rules::IInstruction> instance(factory());
    ASSERT_EQ(instance->name(), std::string(plugin.loadFunction<NameFn>("name")()));
}
    ...
    ::testing::Values(
        ...
        PluginCase{
            ...
            [](utils::encapsulation::SharedObject& plugin) {
                ...
                std::string content = "\x1b[31mTesting\x1b[0m\nS.O.S";
                ...
                ASSERT_EQ(content, "\x1b[31mTesting\x1b[0m\nS.O.S");
            }
        },
        PluginCase{"RuleLn", "rules/rule_ln.so", TYPE_RULE, "ln", [](utils::encapsulation::SharedObject& plugin) {checkRule(plugin, "ln", "Testing\nS.O.S\n");}},
        PluginCase{"RuleDrop", "rules/rule_drop.so", TYPE_RULE, "drop", [](utils::encapsulation::SharedObject& plugin) {checkRule(plugin, "drop", "Testing\nS.O.S\n");}},
        PluginCase{"RuleDup", "rules/rule_dup.so", TYPE_RULE, "dup", [](utils::encapsulation::SharedObject& plugin) {checkRule(plugin, "dup", "Testing\nTesting\n");}},
        PluginCase{"RuleInsert", "rules/rule_insert.so", TYPE_RULE, "insert", [](utils::encapsulation::SharedObject& plugin) {checkRule(plugin, "insert", "Testing\nS.O.S\n");}},
        PluginCase{"RuleReplace", "rules/rule_replace.so", TYPE_RULE, "replace", [](utils::encapsulation::SharedObject& plugin) {checkRule(plugin, "replace", "Testing\nS.O.S\n");}}
    ),
...
TEST(Plugins, DirectoryContent) {
    ...
    ASSERT_EQ(found, (std::set<std::string>{
        "triggers/trigger_default.so",
        "pre-rules/pre-rule_ansi.so",
        "rules/rule_ln.so",
        "rules/rule_drop.so",
        "rules/rule_dup.so",
        "rules/rule_insert.so",
        "rules/rule_replace.so"
    }));
    ASSERT_EQ(names, (std::set<std::string>{"trigger", "ansi", "ln", "drop", "dup", "insert", "replace"}));
}
